...
export function WorkoutBuilder({
  ...
  const handleStart = async () => {
    ...
    try {
      // Auto-save when starting if desired or save first
      ...
      // If save fails, still allow starting if workout is valid
      if (onStart) {
        onStart(workout);
      }
    }
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white pb-24 md:pb-12">
      {/* Top Navigation Bar */}
      <header className="sticky top-0 z-30 bg-zinc-950/80 backdrop-blur-lg border-b border-zinc-800/80 px-4 sm:px-6 py-3.5 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={handleBack}
            aria-label="Back to home"
            className="p-2 -ml-2 rounded-xl text-zinc-400 hover:text-white hover:bg-zinc-800 transition-colors cursor-pointer"
          >
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-zinc-900 border border-zinc-800 hover:border-zinc-700 text-zinc-300 hover:text-white font-bold text-xs uppercase tracking-wider flex items-center gap-1.5 transition-colors cursor-pointer"
          >
          ...
          {onStart && (
            <button
              ...
              className="hidden sm:flex px-4 py-2 rounded-xl bg-white hover:bg-zinc-200 text-zinc-950 font-black text-xs uppercase tracking-wider items-center gap-1.5 transition-all shadow-lg cursor-pointer"
            >
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 pt-6 space-y-6">
        ...
        {saveSuccessMessage && (
          <div
            role="status"
            className="p-3.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-sm font-bold flex items-center gap-2.5 animate-in fade-in slide-in-from-top-2"
          >
        ...
        {validationError && (
          <div
            role="alert"
            className="p-4 rounded-2xl bg-rose-500/10 border border-rose-500/30 text-rose-300 text-sm font-semibold flex items-start gap-3 animate-in fade-in"
          >
            <AlertCircle className="w-5 h-5 text-rose-400 shrink-0 mt-0.5" />
            <div className="flex-1">
              <span className="font-bold uppercase tracking-wide text-xs block text-rose-400 mb-0.5">
                Validation Error
              </span>
        ...
        <section
          ...
          className="bg-zinc-900/80 border border-zinc-800 rounded-3xl p-5 sm:p-7 space-y-6 shadow-xl shadow-black/20"
        >
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label
                ...
                className="text-xs font-black uppercase tracking-wider text-zinc-400 flex items-center justify-between"
              >
              ...
              <input
                ...
                className={`w-full bg-zinc-950/60 border rounded-2xl px-4 py-3 text-lg sm:text-xl font-black text-white placeholder:text-zinc-600 outline-none transition-colors ${
              ...
              />
              {isTitleEmpty && validationError && (
                <p className="text-xs text-rose-400 font-medium">Workout title is required</p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              <label
                ...
                className="text-xs font-bold uppercase tracking-wider text-zinc-400 block"
              >
                Description <span className="text-zinc-500 lowercase">(optional)</span>
              </label>
              <textarea
                id="workout-desc"
                ...
                rows={2}
                className="w-full bg-zinc-950/60 border border-zinc-800 rounded-2xl px-4 py-2.5 text-sm text-zinc-200 placeholder:text-zinc-600 outline-none focus:border-emerald-500 transition-colors resize-none"
              />
            </div>
          </div>

          {/* Global Timers & Rounds Settings */}
          <div className="pt-4 border-t border-zinc-800/80 grid grid-cols-1 md:grid-cols-3 gap-5">
            {/* Prepare Countdown */}
            <DurationInput
        ...
        <section aria-label="Exercises list" className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="px-3.5 py-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 hover:bg-emerald-500/20 text-emerald-400 font-bold text-xs uppercase tracking-wider flex items-center gap-1.5 transition-colors cursor-pointer"
            >
          ...
          {workout.exercises.length === 0 ? (
            <div className="bg-zinc-900/40 border border-dashed border-zinc-800 rounded-3xl p-8 text-center space-y-3">
          ...
          {workout.exercises.length > 0 && (
            <button
              ...
              className="w-full py-4 rounded-2xl border-2 border-dashed border-zinc-800 hover:border-zinc-700 bg-zinc-900/30 hover:bg-zinc-900/60 text-zinc-400 hover:text-zinc-200 font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all cursor-pointer"
            >
      ...
      <div className="sm:hidden fixed bottom-0 inset-x-0 bg-zinc-950/90 backdrop-blur-lg border-t border-zinc-800/80 p-3 flex items-center gap-2 z-30">
        <button
          type="button"
          onClick={handlePreview}
          aria-label="Preview (mobile)"
          className="flex-1 py-3 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-300 font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-1.5"
        >
          <Eye className="w-4 h-4" />
          <span>Preview</span>
        </button>
        ...
        {onStart && (
          <button
            ...
            className="flex-1 py-3 rounded-xl bg-white text-zinc-950 font-black text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 shadow-lg"
          >
            <Play className="w-4 h-4 fill-current" />
            <span>Start</span>
          </button>
