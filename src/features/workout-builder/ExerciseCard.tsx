...
export function ExerciseCard({
  ...
  return (
    <article
      ...
      className="bg-zinc-900/90 border border-zinc-800 rounded-2xl p-4 sm:p-5 shadow-lg shadow-black/20 space-y-4 transition-all"
    >
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2.5 flex-1 min-w-0">
          <button
            ...
            className="p-1 rounded-lg text-zinc-400 hover:text-white hover:bg-zinc-800 transition-transform cursor-pointer"
          >
            <ChevronRight
          ...
          <span className="w-6 h-6 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 text-xs font-black flex items-center justify-center shrink-0">
            {index + 1}
          </span>
          ...
          <input
            ...
            className={`flex-1 min-w-0 bg-transparent text-base sm:text-lg font-bold text-white placeholder:text-zinc-600 outline-none border-b py-1 transition-colors ${
              isNameInvalid
                ? 'border-rose-500 focus:border-rose-400'
                : 'border-transparent hover:border-zinc-700 focus:border-emerald-500'
            }`}
          />
        </div>
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button
            ...
            className="p-2 rounded-xl text-zinc-400 hover:text-white hover:bg-zinc-800 disabled:opacity-20 disabled:cursor-not-allowed transition-colors cursor-pointer"
          >
      ...
      {isNameInvalid && (
        <p className="text-xs text-rose-400 font-medium pl-8" role="alert">
          Exercise name is required
        </p>
      )}

      {/* Expanded Exercise Configuration Fields */}
      {isExpanded && (
        <div className="pt-2 border-t border-zinc-800/80 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              >
                Target Reps <span className="text-zinc-500 lowercase">(optional)</span>
              </label>
              <input
                ...
                className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-3.5 py-2 text-base font-bold text-white placeholder:text-zinc-600 outline-none focus:border-emerald-500 transition-colors"
              />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-3.5 py-2 text-sm text-zinc-200 placeholder:text-zinc-600 outline-none focus:border-emerald-500 transition-colors"
            />
          </div>
        </div>
      )}
